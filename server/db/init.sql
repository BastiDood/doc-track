...
CREATE DOMAIN GlobalPermission AS BIT VARYING(9) NOT NULL DEFAULT B'0';
...
CREATE TABLE snapshot(
    ...
    target SMALLINT
        ...
        DEFAULT NULL
        CHECK((status IN ('Send', 'Receive') AND target IS NOT NULL) OR (status NOT IN ('Send', 'Receive') AND target IS NULL)),
